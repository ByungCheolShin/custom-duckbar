...
그러면 `.build/app/DuckBar.app`이 생성됩니다.
...
바이너리는 `.build/release/DuckBar`에 생성됩니다.
...
1. 빌드된 `DuckBar.app` 파일을 찾습니다
2. `/Applications` 폴더로 드래그 드롭합니다
3. Spotlight(Cmd+Space)에서 "DuckBar" 검색 후 실행
...
- **없음**: 순수 Swift 표준 라이브러리와 Apple 기본 프레임워크만 사용
